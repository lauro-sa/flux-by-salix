  ...
  try {
    ...
    switch (accion) {
      ...
      case 'crear_tipo': {
        ...
        const { data, error } = await admin
          ...
          .insert({
            ...
            siguiente_tipo_id: datos.siguiente_tipo_id || null,
            tipo_encadenamiento: datos.tipo_encadenamiento || 'sugerir',
      ...
      case 'reordenar_tipos': {
        ...
        if (errores.length > 0) {
          console.error('Errores al reordenar tipos actividad:', errores.map(e => e.error))
      ...
      case 'reordenar_estados': {
        ...
        const errores = resultados.filter(r => r.error)
        if (errores.length > 0) {
          console.error('Errores al reordenar estados actividad:', errores.map(e => e.error))
          return NextResponse.json({ error: 'Error parcial al reordenar' }, { status: 500 })
        }
        return NextResponse.json({ ok: true })
      }
