...
// PATCH /api/inbox/respuestas-whatsapp/[id] — actualizar con auditoría campo por campo
export async function PATCH(
  ...
  try {
    ...
    if (cambios.length > 0) {
      ...
      await admin.from('auditoria_respuestas_rapidas_whatsapp').insert(auditorias)
    }
    ...
    console.error('Error al actualizar respuesta de WhatsApp:', err)
    return NextResponse.json({ error: 'Error al actualizar respuesta' }, { status: 500 })
  }
}

// DELETE /api/inbox/respuestas-whatsapp/[id]
export async function DELETE(
  ...
  try {
    ...
    console.error('Error al eliminar respuesta de WhatsApp:', err)
