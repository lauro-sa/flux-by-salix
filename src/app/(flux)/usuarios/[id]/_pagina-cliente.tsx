...
interface PropsPagina {
  miembroId: string
  /** Nombre del miembro/perfil precargado server-side. Se muestra en el
   *  cargador (CargaIcono) mientras se completa la hidratación cliente. */
  nombreInicial: string | null
}

export default function PaginaPerfilUsuario({ miembroId, nombreInicial }: PropsPagina) {
  ...
  // Mantiene activa la BarraProgresoGlobal del header mientras el fetch
  // inicial corre — única señal visible durante la carga (sin skeleton placebo).
  ...
  /* ════════════ LOADING / ERROR ════════════
     Ícono de usuario dibujándose + nombre real (precargado server-side o
     ya resuelto en cliente). Mismo patrón unificado del resto de detalles. */
  if (cargando) {
    const nombreLoader =
      (perfil ? [perfil.nombre, perfil.apellido].filter(Boolean).join(' ').trim() : '') ||
      nombreInicial ||
      undefined
    return <CargaIcono icono={<CircleUserRound size={52} strokeWidth={1} />} nombre={nombreLoader} />
  }
