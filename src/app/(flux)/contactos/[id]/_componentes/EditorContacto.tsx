...
// ═══════════════════════════════════════════════════════════════
// EDITOR: /contactos/[id] — Detalle, edición y creación unificados
// ═══════════════════════════════════════════════════════════════

/** Datos opcionales precargados por el Server Component (`page.tsx`).
 *  Si están presentes, el editor arranca sin el flash de "cargando" y
 *  sin disparar los fetches que el componente hacía al montar. */
export interface DatosInicialesContacto {
  tipos?: {
    tipos_contacto?: TipoContacto[]
    tipos_relacion?: TipoRelacion[]
    puestos_vinculacion?: { id: string; etiqueta: string }[]
    campos_fiscales?: CampoFiscalPais[]
    paises?: string[]
  }
  config?: {
    etiquetas?: Array<{ nombre?: string; color?: string; activo?: boolean }>
    rubros?: Array<{ nombre?: string; activo?: boolean }>
  }
  contacto?: Record<string, unknown> | null
}

interface PropsEditorContacto {
  datosIniciales?: DatosInicialesContacto
}

export default function EditorContacto({ datosIniciales }: PropsEditorContacto = {}) {
  return (
    <GuardPagina modulo="contactos">
      <EditorContactoInterno datosIniciales={datosIniciales} />
    </GuardPagina>
  )
}
...
function EditorContactoInterno({ datosIniciales }: PropsEditorContacto) {
  ...
  // ─── Datos precargados desde el server ─────────────────────────────
  // Si el page.tsx (Server Component) precargó tipos / config / contacto,
  // los usamos como estado inicial y skipeamos los fetches. Si no, los
  // useState arrancan vacíos y los useEffects de abajo hacen la carga
  // como antes (modo retrocompatible).
  const contactoInicial = datosIniciales?.contacto || null
  const tiposIniciales = datosIniciales?.tipos
  const configIniciales = datosIniciales?.config
  const tieneContactoInicial = !!contactoInicial

  // ─── Estado compartido ───
  const [nombreCompleto, setNombreCompleto] = useState(() =>
    contactoInicial ? combinarNombre((contactoInicial.nombre as string) || '', (contactoInicial.apellido as string | null) || null) : ''
  )
  const [codigo, setCodigo] = useState<string>(() => (contactoInicial?.codigo as string) || '')
  ...
  const [tipoContactoId, setTipoContactoId] = useState<string>(() => (contactoInicial?.tipo_contacto_id as string) || '')
  const [campos, setCampos] = useState<Record<string, string>>(() => {
    if (!contactoInicial) return {} as Record<string, string>
    const ci = contactoInicial
    return {
      titulo: (ci.titulo as string) || '',
      correo: (ci.correo as string) || '',
      web: (ci.web as string) || '',
      cargo: (ci.cargo as string) || '',
      rubro: (ci.rubro as string) || '',
      ...
      notas: (ci.notas as string) || '',
    }
  })
  const [datosFiscales, setDatosFiscales] = useState<Record<string, string>>(() =>
    (contactoInicial?.datos_fiscales as Record<string, string>) || {}
  )
  const [etiquetas, setEtiquetas] = useState<string[]>(() => (contactoInicial?.etiquetas as string[]) || [])
  const [direcciones, setDirecciones] = useState<DireccionConTipo[]>(() => {
    const dirs = (contactoInicial?.direcciones as Array<Record<string, unknown>>) || []
    return dirs.map((d) => ({
  ...
  const [telefonos, setTelefonos] = useState<TelefonoNormalizado[]>(() => {
    const tels = (contactoInicial?.telefonos as Array<Record<string, unknown>>) || []
    return tels.map((tel) => ({
      tipo: ((tel.tipo as TelefonoNormalizado['tipo']) || 'movil'),
      valor: (tel.valor as string) || '',
      es_whatsapp: !!tel.es_whatsapp,
      es_principal: !!tel.es_principal,
      etiqueta: (tel.etiqueta as string | null) || null,
      orden: typeof tel.orden === 'number' ? tel.orden : 0,
    }))
  })
  ...
  const [miembroIdVinculado, setMiembroIdVinculado] = useState<string | null>(() =>
    (contactoInicial?.miembro_id as string | null) || null
  )
  const [tiposContacto, setTiposContacto] = useState<TipoContacto[]>(() =>
    tiposIniciales?.tipos_contacto?.length ? tiposIniciales.tipos_contacto : []
  )
  const [tiposRelacion, setTiposRelacion] = useState<TipoRelacion[]>(() =>
    tiposIniciales?.tipos_relacion || []
  )
  const [puestosVinculacion, setPuestosVinculacion] = useState<{ id: string; etiqueta: string }[]>(() =>
    tiposIniciales?.puestos_vinculacion || []
  )
  const [etiquetasConfig, setEtiquetasConfig] = useState<{ nombre: string; color: string }[]>(() =>
    (configIniciales?.etiquetas || [])
      .filter(e => e.activo !== false)
      .map(e => ({ nombre: (e.nombre as string) || '', color: (e.color as string) || 'neutro' }))
  )
  const [rubrosConfig, setRubrosConfig] = useState<{ nombre: string }[]>(() =>
    (configIniciales?.rubros || [])
      .filter(r => r.activo !== false)
      .map(r => ({ nombre: (r.nombre as string) || '' }))
  )
  const [camposFiscalesPais, setCamposFiscalesPais] = useState<CampoFiscalPais[]>(() =>
    tiposIniciales?.campos_fiscales || []
  )
  const [paisesEmpresa, setPaisesEmpresa] = useState<string[]>(() =>
    tiposIniciales?.paises || []
  )
  const [paisContacto, setPaisContacto] = useState<string>(() => {
    if (contactoInicial?.pais_fiscal) return contactoInicial.pais_fiscal as string
    if (tiposIniciales?.paises?.length === 1) return tiposIniciales.paises[0]
    return ''
  })
  ...
  const [vinculaciones, setVinculaciones] = useState<unknown[]>(() =>
    (contactoInicial?.vinculaciones as unknown[]) || []
  )
  const [vinculacionesInversas, setVinculacionesInversas] = useState<unknown[]>(() =>
    (contactoInicial?.vinculaciones_inversas as unknown[]) || []
  )
  const [avatarUrl, setAvatarUrl] = useState<string | null>(() =>
    (contactoInicial?.avatar_url as string | null) || null
  )
  ...
  // Cargar tipos + config (compartido). Si el Server Component ya los pasó
  // como props, saltamos el fetch — los useState arrancaron con esos datos.
  useEffect(() => {
    // Si ya tenemos tipos del server, solo preseleccionar persona en modo
    // creación cuando todavía no hay tipo seleccionado.
    if (tiposIniciales) {
      if (esNuevo && !tipoContactoId && tiposContacto.length > 0) {
        const persona = tiposContacto.find(t => t.clave === 'persona')
        if (persona) {
          setTipoContactoId(persona.id)
          const paisInicial = paisesEmpresa.length === 1 ? paisesEmpresa[0] : ''
  ...
  // Si el Server Component precargó el contacto, saltamos el fetch — los
  // useState arrancaron con esos datos. Solo aplicamos la migaja dinámica.
  useEffect(() => {
    if (esNuevo) return
    ...
    <div className="flex flex-col h-full overflow-auto">
      ...
      {!esNuevo && esProvisorio && (
        <ModalAceptarProvisorio
          ...
          contactoId={contactoId!}
          nombreContacto={nombreCompleto}
          onAceptarNuevo={(cod: string | null) => {
            setEsProvisorio(false)
