 ...
 * Mismo patrón que WhatsApp (useEstadoWhatsApp).
 *
...
export function useEstadoInbox() {
  ...
  // consultar en el mismo render inicial. Sin esto, en el primer render se
  // verían los defaults (`canalCorreoActivo=''`, `carpetaCorreo='entrada'`)
  // y el cache existiría pero no se usaría hasta que terminen los fetches.
  ...
  // quede "en blanco" mientras llega el fetch.
  useEffect(() => {
   ...
   *  tenga que pasar este handler. Acciones específicas de WhatsApp
   *  (silenciar, pipeline, bloquear, fijar_para_usuario) se silencian
   *  porque el menú las oculta cuando tipoCanal !== 'whatsapp'. */
