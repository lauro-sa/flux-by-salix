            ...
            {canalesOrdenados.map((canal) => {
              ...
              return (
                <Reorder.Item key={canal.id} value={canal.id} className={`${esMovil ? 'mb-2' : 'mb-1'} list-none`}>
                  {/* Header del canal: tap en cualquier parte (nombre o chevron)
                      solo expande/colapsa. NO navega a la cuenta — para eso
                      están las carpetas (Bandeja, Enviados, Spam, Archivar). */}
                  <Boton
                    variante="fantasma"
                    tamano={tamanoBtn}
                    anchoCompleto
                          ...
                          return (
                            <Fragment key={carpeta.clave}>
                              {/* Separador "inset" estilo iOS: empieza donde
                                  empieza el texto (después del ícono) y termina
                                  donde termina el chevron. ml-[42px] = pl-4
                                  (16px) + ícono 14px + gap-3 (12px). mr-3 = pr-3
                                  del botón → alineado con el borde del chevron. */}
                              {esMovil && idx > 0 && (
                                <div
                              ...
                              <span className={`flex items-center ${esMovil ? 'gap-3' : 'gap-2'} w-full`}>
                                ...
                                {/* Conteo: pill en desktop, número plano en móvil (estilo iOS) */}
                                {total > 0 && (
                                  esMovil ? (
                                    <span
