...
/**
 * Listado de respuestas rápidas de WhatsApp — pantalla completa.
 * Mismo patrón que respuestas de correo pero canal fijo WhatsApp.
 */
...
export default function PaginaListadoRespuestasWhatsApp() {
  ...
  const handleEliminar = async (p: PlantillaRespuesta) => {
    setEliminando(true)
    try {
      await fetch(`/api/whatsapp/respuestas-rapidas/${p.id}`, { method: 'DELETE' })
  ...
  const eliminarLote = async (ids: Set<string>) => {
    ...
    try {
      await Promise.all(aEliminar.map(p => fetch(`/api/whatsapp/respuestas-rapidas/${p.id}`, { method: 'DELETE' })))
  ...
  const handleDuplicar = async (p: PlantillaRespuesta) => {
    try {
      await fetch('/api/whatsapp/respuestas-rapidas', {
        method: 'POST',
  ...
  const handleExportar = () => {
    ...
    a.href = url
    a.download = `respuestas-whatsapp-${new Date().toISOString().slice(0, 10)}.json`
    a.click()
  ...
  const handleReordenar = async (idsOrdenados: string[]) => {
    ...
    try {
      await Promise.all(nuevas.map(p =>
        fetch(`/api/whatsapp/respuestas-rapidas/${p.id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
      ...
      render: (p) => (
        <div className="flex items-center gap-2 min-w-0">
          <div
            ...
            style={{
              backgroundColor: 'var(--insignia-exito-fondo)',
    ...
    },
    {
      ...
      render: (p) => (p.editado_por || p.creado_por) ? (
        <IndicadorEditado
          ...
          tablaAuditoria="auditoria_respuestas_rapidas_whatsapp"
    ...
    <>
      <PlantillaListado
        ...
        accionPrincipal={{
          ...
          icono: <Plus size={14} />,
          onClick: () => router.push('/whatsapp/configuracion/respuestas-rapidas/nueva'),
        }}
        acciones={[
          {
            id: 'exportar',
        ...
        <TablaDinamica
          ...
          estadoVacio={
            <EstadoVacio
              ...
              accion={!cargando
                ? (
                  <Boton onClick={() => router.push('/whatsapp/configuracion/respuestas-rapidas/nueva')}>
