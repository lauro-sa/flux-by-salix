...
/**
 * SelectorContacto — Buscador de contacto reutilizable.
 *
 * Se usa en: presupuestos, visitas, facturas, informes, etc.
 * - Input plano con chevron
 * - "RECIENTES" al enfocar sin texto
 * - Resultados: nombre + badge tipo + código + correo · teléfono
 * - Jerarquía padre-hijo expandible
 * - Ficha compacta al seleccionar + renderFicha para contenido custom
 */

// ── Tipos exportados ──

export interface ContactoHijo {
  id: string
  nombre: string
...
export interface ContactoResultado {
  ...
  direcciones: { id?: string; texto: string | null; tipo?: string; es_principal: boolean; total_visitas?: number; ultima_visita?: string | null }[]
...
interface PropiedadesSelectorContacto {
  ...
  /** Contenido extra a renderizar debajo de la ficha del contacto seleccionado */
  renderExtra?: (contacto: ContactoSeleccionado) => ReactNode
  /** Ocultar alerta de sin correo (ej: en visitas no importa) */
  sinAlertaCorreo?: boolean
  /** Ocultar datos fiscales (ej: en visitas no importa) */
...
function SelectorContacto({
  ...
  useEffect(() => {
    ...
    const cerrar = (e: MouseEvent) => {
      if (refContenedor.current && !refContenedor.current.contains(e.target as Node)) {
        // También verificar si el click fue dentro del portal
        const portal = document.getElementById('selector-contacto-portal')
      ...
      <div className={`rounded-card bg-superficie-app/50 px-3 py-3 ${error ? 'ring-2 ring-estado-error/50' : ''}`}>
        <div className="flex items-start justify-between">
          <div className="space-y-1.5">
            ...
            {!sinDatosFiscales && contacto.numero_identificacion && (
                ...
                <div className="flex items-center gap-2">
                  ...
                  {direcciones.length > 1 && !soloLectura && (
                    <select
                      ...
                      onChange={(e) => {
                        if (onCambiarDireccion) onCambiarDireccion(e.target.value)
                      }}
                    >
                      {direcciones.map((d, i) => (
          ...
          <div className="flex items-center gap-1 shrink-0">
            {!soloLectura && (
              <Boton variante="fantasma" tamano="xs" soloIcono icono={<X size={14} />} onClick={limpiar} titulo="Cambiar contacto" className="text-texto-terciario hover:text-estado-error hover:bg-estado-error/10" />
            )}
            {contacto.id && (
        ...
        {/* Contenido extra (direcciones para visitas, etc.) */}
        {renderExtra && renderExtra(contacto)}
      </div>
    )
  }

  // ── Sin contacto: buscador ──
  if (soloLectura) {
    return <p className="text-sm text-texto-terciario py-2">Sin contacto asignado</p>
  }
  ...
  return (
    <div ref={refContenedor} className="relative">
      ...
      {/* Dropdown — portal para salir del overflow del modal */}
      {typeof document !== 'undefined' && createPortal(
        <AnimatePresence>
          {abierto && listaVisible.length > 0 && posDropdown && (
            <motion.div
              id="selector-contacto-portal"
