...
function PlantillaListado({
  ...
  return (
    <div className={`flex flex-col h-full ${className}`}>
      ...
      {/* ═══ CABECERO — botones izquierda, título derecha ═══ */}
      <div className="flex items-center gap-3 shrink-0 px-4 sm:px-6 pt-4 sm:pt-5 pb-3">

        {/* Lado izquierdo: acción principal + acciones */}
        <div className="flex items-center gap-2">
          {accionPrincipal && (
            <button
              type="button"
              onClick={accionPrincipal.onClick}
              className="flex items-center justify-center gap-2 px-4 h-9 rounded-lg text-sm font-semibold text-texto-inverso cursor-pointer border-none transition-colors hover:opacity-90"
          ...
          {acciones.length > 0 && (
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1.5 px-3 h-9 rounded-lg border border-borde-sutil bg-transparent text-sm font-medium text-texto-secundario hover:bg-superficie-hover cursor-pointer transition-colors"
              >
                Acciones
        ...
        <div className="flex items-center gap-2">
          ...
          {mostrarConfiguracion && onConfiguracion && (
            <button
              ...
              className="size-8 inline-flex items-center justify-center rounded-md hover:bg-superficie-hover cursor-pointer transition-colors text-texto-terciario hover:text-texto-secundario border-none bg-transparent"
              title="Configuración"
            >
              <Settings size={16} />
            </button>
