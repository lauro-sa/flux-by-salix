...
export function ModalAgregarCanal({ abierto, onCerrar, tipoCanal, onCanalCreado, canalEditar, propietarioUsuarioId }: PropiedadesModal) {
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      let res: Response
      // Endpoint según tipo de canal (modularidad: correo y whatsapp tienen APIs separadas)
      const rutaCanales = tipoCanal === 'whatsapp'
      ...
      if (modoEdicion && canalEditar) {
        ...
        res = await fetch(`${rutaCanales}/${canalEditar.id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            nombre: nombre.trim(),
  ...
  return (
    <Modal
      ...
      tamano="lg"
      accionPrimaria={paso === 1 ? {
        etiqueta: 'Siguiente',
        onClick: () => proveedor ? setPaso(2) : setError('Seleccioná un proveedor'),
        disabled: !proveedor,
      } : {
        etiqueta: modoEdicion ? 'Guardar cambios' : 'Conectar',
