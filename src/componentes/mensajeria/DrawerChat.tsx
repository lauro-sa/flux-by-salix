      ...
      {abierto && conversacion && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-4 right-4 bottom-4 z-50 flex flex-col w-[420px] max-w-[calc(100vw-2rem)] rounded-modal overflow-hidden shadow-2xl border border-borde-sutil"
                    ...
                    {grupo.mensajes.map((msg) => {
                      ...
                      if (esNota) {
                        return (
                          <div key={msg.id} className="flex justify-center my-1">
                            <div className="text-xs text-yellow-600 dark:text-yellow-400 bg-yellow-500/10 border border-yellow-500/20 rounded-card px-3 py-1 max-w-[90%] text-center italic">
                              {msg.texto}
                            </div>
                          </div>
                        )
                      ...
                      return (
                        <div key={msg.id} className={`flex ${esEntrante ? 'justify-start' : 'justify-end'} mb-0.5`}>
                          <div
                            className={`max-w-[85%] rounded-card px-3 py-1.5 text-sm leading-relaxed ${
                              esEntrante
