...
function TarjetaConversacion({ conversacion }: { conversacion: ConversacionConDetalles }) {
  ...
  return (
    <div className="rounded-card border p-3 bg-superficie-tarjeta border-borde-sutil hover:border-borde-fuerte transition-colors">
  ...
  return (
    ...
    >
      ...
      <SortableContext items={idsItems} strategy={horizontalListSortingStrategy}>
        <div
          className={`flex flex-col gap-2 min-h-[120px] rounded-card p-1.5 transition-all duration-200 ${
            sobreMi
        ...
        >
          ...
          {sobreMi && items.length === 0 && (
            <div className="text-xs text-texto-marca text-center py-8 border-2 border-dashed border-texto-marca/50 rounded-card font-medium">
              Soltar aquí
            </div>
          )}
        </div>
      ...
      {Array.from({ length: 4 }).map((_, i) => (
        <div key={i} className="flex flex-col gap-3 min-w-[260px] sm:min-w-[280px] w-[260px] sm:w-[280px] shrink-0">
          ...
          {Array.from({ length: 3 - i }).map((_, j) => (
            <div key={j} className="rounded-card border border-borde-sutil p-3 space-y-2">
    ...
    <div className="flex flex-col gap-4 h-full">
      ...
      {cargando ? (
        ...
        >
          ...
          <DragOverlay dropAnimation={{ duration: 200, easing: 'ease' }}>
            {activeDragConv && (
              <div className="rotate-2 scale-105 shadow-lg rounded-card">
                <TarjetaConversacion conversacion={activeDragConv} />
              </div>
            )}
            {activeDragCol && (
              <div className="bg-superficie-elevada border border-borde-fuerte rounded-card px-3 py-2 shadow-lg flex items-center gap-2">
                <GripVertical size={14} className="text-texto-terciario" />
        ...
        <div className="flex flex-col gap-3">
          ...
          <ul className="flex flex-col gap-2">
            {modalValidacion?.resultado.estrictos.map((req) => (
              <li key={req.campo} className="flex items-center gap-2 text-sm text-texto-secundario bg-insignia-peligro/8 rounded-card px-3 py-2">
      ...
      >
        <div className="flex flex-col gap-3">
          ...
          <ul className="flex flex-col gap-2">
            {modalValidacion?.resultado.recomendados.map((req) => (
              <li key={req.campo} className="flex items-center gap-2 text-sm text-texto-secundario bg-insignia-advertencia/8 rounded-card px-3 py-2">
                <span className="text-base">{req.icono}</span>
                <span>{req.nombre}</span>
              </li>
            ))}
      ...
      >
        <div className="flex flex-col gap-3">
          ...
          <textarea
            className="w-full rounded-card border border-borde-sutil bg-superficie-tarjeta text-texto-primario text-sm px-3 py-2.5 min-h-[100px] resize-y focus:outline-none focus:ring-2 focus:ring-texto-marca/30 focus:border-texto-marca transition-colors placeholder:text-texto-terciario"
