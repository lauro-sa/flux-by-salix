 ...
 * Carga plantillas desde el endpoint del canal correspondiente.
...
export function SelectorRespuestasRapidas({
  ...
  // Cargar plantillas cuando se abre el selector — el endpoint depende del canal
  useEffect(() => {
    if (!visible) return
    const ruta = canal === 'whatsapp'
      ? '/api/whatsapp/respuestas-rapidas'
      : canal === 'correo'
        ? '/api/correo/respuestas-rapidas'
        : null
    if (!ruta) {
      setPlantillas([])
      return
    ...
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          className="absolute bottom-full left-0 right-0 mx-3 mb-1 rounded-card overflow-hidden shadow-lg z-20"
          style={{
